...
for filename in os.listdir(awards_dir):
      if filename.endswith(".html"):
        ...
        table = soup.find("table", {"id": "leading_all_defense"})

        if not table:
            print(f"❌ No All-Defense table found in {filename}")
            continue
...
combined_df.to_csv("backend/data/all_defense_votes_combined.csv", index=False)
...
all_defense_df = pd.read_csv("backend/data/all_defense_votes_combined.csv")  # Your cleaned All-NBA file
all_defense_df["Name"] = all_defense_df["Name"].str.strip()
all_defense_df["Year"] = all_defense_df["Year"].astype(int)
all_defense_df["Selection"] = pd.to_numeric(all_defense_df["Selection"], errors="coerce").fillna(0).astype(int)
...
merged_df = pd.merge(
    player_df,
    all_defense_df[["Name", "Year", "Selection"]],
    on=["Name", "Year"],
    how="left"
)

merged_df["Selection"] = merged_df["Selection"].fillna(0).astype(int)
...
merged_df.to_csv("backend/data/all_defense_training_data.csv", index=False)
print("✅ Saved combined All-Defense training data to: backend/data/all_defense_training_data.csv")
